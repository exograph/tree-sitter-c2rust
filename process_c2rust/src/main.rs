            ...
            syn::Expr::MethodCall(m) => {
                if m.method.to_token_stream().to_string() == "expect" {
                    if m.receiver.to_token_stream().to_string() == "ts_current_free" {
                        *i = syn::parse_quote!(crate::core_transpiled::alloc::ts_free);
                    } else if m.receiver.to_token_stream().to_string() == "ts_current_malloc" {
                        *i = syn::parse_quote!(crate::core_transpiled::alloc::ts_malloc);
                    } else if m.receiver.to_token_stream().to_string() == "ts_current_calloc" {
                        *i = syn::parse_quote!(crate::core_transpiled::alloc::ts_calloc);
                    } else if m.receiver.to_token_stream().to_string() == "ts_current_realloc" {
                        *i = syn::parse_quote!(crate::core_transpiled::alloc::ts_realloc);
                    }
                }
            }
            _ => {}
                ...
                .starts_with("C2RustUnnamed_")
            {
                    ...
                    ]
                {
                    let contents_type = fields
                        ...
                        .ty
                        .clone();
                    *i = syn::parse_quote!(type #name = crate::core_transpiled::util::StackElement<#contents_type>;);
                } else if field_names
                    == vec![
                ...
                {
                    *i = syn::parse_quote!(type #name = crate::core_transpiled::util::LongShortData;);
                } else if field_names
                    == vec![
                ...
                {
                    *i = syn::parse_quote!(type #name = crate::core_transpiled::util::ScannerStateWithLookahead;);
                } else if field_names
                    == vec![
                ...
                {
                    *i = syn::parse_quote!(type #name = crate::core_transpiled::util::ScannerStateLookaheadMeta;);
                } else if field_names
                    == vec![
                ...
                {
                    *i = syn::parse_quote!(type #name = crate::core_transpiled::util::ScannerStateLookaheadFirstLeaf;);
                }
            }
        }
...
fn main() {
    ...
    public_apis.insert("lib/src/lexer.rs", vec!["Lexer", "ColumnData"]);
    public_apis.insert(
        "lib/src/subtree.rs",
        vec![
            "Subtree",
    ...
    for file in public_apis.keys() {
        ...
        let fd_functions = ["ts_parser_print_dot_graphs", "ts_tree_print_dot_graph"];
                        ...
                        if i == "_IO_FILE" {
                            ...
                            if i == "FILE" {
                                *item = syn::parse_quote!(
                                    use crate::core_transpiled::util::libc::{
                ...
                if let syn::Item::Const(syn::ItemConst { ident: i, ty, .. }) = item {
                    if !ty.to_token_stream().to_string().starts_with("C2Rust") {
                        let new_name = syn::Ident::new(
                            &format!("{}_{}", ty.to_token_stream(), i.to_token_stream()),
                            i.span(),
                        );
