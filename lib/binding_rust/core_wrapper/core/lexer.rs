...
#[inline]
unsafe extern "C" fn ts_decode_utf16_be(
    ...
    let fresh0 = i;
    i = i.wrapping_add(1);
    *code_point = __bswap_16(*(string as *mut uint16_t).offset(fresh0 as isize)) as int32_t;
...
#[inline]
unsafe extern "C" fn ts_decode_utf16_le(
    mut string: *const uint8_t,
    mut length: uint32_t,
    mut code_point: *mut int32_t,
) -> uint32_t {
    ...
    let fresh1 = i;
    i = i.wrapping_add(1);
    *code_point = __uint16_identity(*(string as *mut uint16_t).offset(fresh1 as isize)) as int32_t;
    if *code_point as libc::c_uint & 0xfffffc00 as libc::c_uint
        == 0xd800 as libc::c_int as libc::c_uint
    {
        let mut __c2: uint16_t = 0;
...
unsafe extern "C" fn ts_decode_utf8(
    ...
    let fresh2 = i;
    i = i.wrapping_add(1);
    *code_point = *string.offset(fresh2 as isize) as int32_t;
...
unsafe extern "C" fn ts_lexer__set_column_data(mut self_0: *mut Lexer, mut val: uint32_t) {
    (*self_0).column_data.valid = 1 as libc::c_int != 0;
    (*self_0).column_data.value = val;
}
unsafe extern "C" fn ts_lexer__increment_column_data(mut self_0: *mut Lexer) {
    if (*self_0).column_data.valid {
        (*self_0).column_data.value = ((*self_0).column_data.value).wrapping_add(1);
    }
}
unsafe extern "C" fn ts_lexer__invalidate_column_data(mut self_0: *mut Lexer) {
    (*self_0).column_data.valid = 0 as libc::c_int != 0;
    (*self_0).column_data.value = 0 as libc::c_int as uint32_t;
}
...
unsafe extern "C" fn ts_lexer__get_lookahead(mut self_0: *mut Lexer) {
    ...
    let mut decode: DecodeFunction = if (*self_0).input.encoding as libc::c_uint
    ...
    } else if (*self_0).input.encoding as libc::c_uint
        == TSInputEncodingUTF16LE as libc::c_int as libc::c_uint
    {
        Some(
            ts_decode_utf16_le
                as unsafe extern "C" fn(*const uint8_t, uint32_t, *mut int32_t) -> uint32_t,
        ...
        == TSInputEncodingUTF16BE as libc::c_int as libc::c_uint
    {
        Some(
            ts_decode_utf16_be
                as unsafe extern "C" fn(*const uint8_t, uint32_t, *mut int32_t) -> uint32_t,
...
unsafe extern "C" fn ts_lexer_goto(mut self_0: *mut Lexer, mut position: Length) {
    if position.bytes != (*self_0).current_position.bytes {
        ts_lexer__invalidate_column_data(self_0);
    }
...
unsafe extern "C" fn ts_lexer__do_advance(mut self_0: *mut Lexer, mut skip: bool) {
    if (*self_0).lookahead_size != 0 {
        if (*self_0).data.lookahead == '\n' as i32 {
            ...
            ts_lexer__set_column_data(self_0, 0 as libc::c_int as uint32_t);
        } else {
            let mut is_bom: bool = (*self_0).current_position.bytes
                == 0 as libc::c_int as libc::c_uint
                && (*self_0).data.lookahead == BYTE_ORDER_MARK;
            if !is_bom {
                ts_lexer__increment_column_data(self_0);
            }
...
unsafe extern "C" fn ts_lexer__get_column(mut _self: *mut TSLexer) -> uint32_t {
    ...
    return (*self_0).column_data.value;
}
...
pub unsafe extern "C" fn ts_lexer_init(mut self_0: *mut Lexer) {
    *self_0 = {
        let mut init = Lexer {
            ...
            lookahead_size: 0,
            did_get_column: 0 as libc::c_int != 0,
            column_data: {
                let mut init = ColumnData {
...
pub unsafe extern "C" fn ts_lexer_start(mut self_0: *mut Lexer) {
    ...
    if !ts_lexer__eof(&mut (*self_0).data) {
        ...
        if (*self_0).current_position.bytes == 0 as libc::c_int as libc::c_uint {
            if (*self_0).data.lookahead == BYTE_ORDER_MARK {
