...
pub struct TSLexerMode {
    ...
    pub reserved_word_set_id: uint16_t,
}
#[derive(Copy, Clone)]
#[repr(C)]
...
pub struct TSParser {
    ...
    pub parse_options: TSParseOptions,
    pub parse_state: TSParseState,
    pub included_range_difference_index: libc::c_uint,
    pub has_scanner_error: bool,
    pub canceled_balancing: bool,
    pub has_error: bool,
}
#[derive(Copy, Clone)]
#[repr(C)]
...
unsafe extern "C" fn length_sub(mut len1: Length, mut len2: Length) -> Length {
    ...
    result.bytes = if len1.bytes >= len2.bytes {
        (len1.bytes).wrapping_sub(len2.bytes)
    } else {
        0 as libc::c_int as libc::c_uint
    };
...
static mut MAX_COST_DIFFERENCE: libc::c_uint =
    (18 as libc::c_int * 100 as libc::c_int) as libc::c_uint;
...
unsafe extern "C" fn ts_parser__call_keyword_lex_fn(mut self_0: *mut TSParser) -> bool {
    if ts_language_is_wasm((*self_0).language) {
...
unsafe extern "C" fn ts_parser__can_reuse_first_leaf(
    ...
    let mut current_lex_mode: TSLexerMode =
        ts_language_lex_mode_for_state((*self_0).language, state);
    let mut leaf_lex_mode: TSLexerMode =
        ts_language_lex_mode_for_state((*self_0).language, leaf_state);
    ...
    if (*table_entry).action_count > 0 as libc::c_int as libc::c_uint
        && if std::slice::from_raw_parts(
            &mut leaf_lex_mode as *mut TSLexerMode as *const u8,
            ::core::mem::size_of::<TSLexerMode>() as libc::c_ulong,
        ) == std::slice::from_raw_parts(
            &mut current_lex_mode as *mut TSLexerMode as *const u8,
            ::core::mem::size_of::<TSLexerMode>() as libc::c_ulong,
        ) {
            0
        } else {
...
unsafe extern "C" fn ts_parser__lex(
    ...
    let mut lex_mode: TSLexerMode = ts_language_lex_mode_for_state((*self_0).language, parse_state);
    ...
    loop {
        ...
        let mut column_data: ColumnData = (*self_0).lexer.column_data;
        if lex_mode.external_lex_state as libc::c_int != 0 as libc::c_int {
            ...
            if found_token {
                ...
                {
                    let mut symbol: TSSymbol = *((*(*self_0).language).external_scanner.symbol_map)
                        .offset((*self_0).lexer.data.result_symbol as isize);
                    let mut next_parse_state: TSStateId =
                        ts_language_next_state((*self_0).language, parse_state, symbol);
                    let mut token_is_extra: bool =
                        next_parse_state as libc::c_int == parse_state as libc::c_int;
                    if error_mode as libc::c_int != 0
                        || !ts_stack_has_advanced_since_error((*self_0).stack, version)
                        || token_is_extra as libc::c_int != 0
                    {
                        if ((*self_0).lexer.logger.log).is_some()
            ...
            if found_token {
                ...
                (*self_0).lexer.column_data = column_data;
            }
        }
        if ((*self_0).lexer.logger.log).is_some() || !((*self_0).dot_graph_file).is_null() {
            snwrite!(
        ...
        if !error_mode {
            ...
            lex_mode =
                ts_language_lex_mode_for_state((*self_0).language, 0 as libc::c_int as TSStateId);
            ts_lexer_reset(&mut (*self_0).lexer, start_position);
        } else {
            if !skipped_error {
                if ((*self_0).lexer.logger.log).is_some() || !((*self_0).dot_graph_file).is_null() {
    ...
    if skipped_error {
        ...
        let mut symbol_0: TSSymbol = (*self_0).lexer.data.result_symbol;
        ...
        if found_external_token {
            symbol_0 =
                *((*(*self_0).language).external_scanner.symbol_map).offset(symbol_0 as isize);
        } else if symbol_0 as libc::c_int
            == (*(*self_0).language).keyword_capture_token as libc::c_int
            && symbol_0 as libc::c_int != 0 as libc::c_int
        {
            ...
            ts_lexer_start(&mut (*self_0).lexer);
            is_keyword = ts_parser__call_keyword_lex_fn(self_0);
            if is_keyword as libc::c_int != 0
                ...
                && (ts_language_has_actions(
                    ...
                    || ts_language_is_reserved_word(
                        ...
                        != 0)
            {
                symbol_0 = (*self_0).lexer.data.result_symbol;
            }
        }
    ...
    if ((*self_0).lexer.logger.log).is_some() || !((*self_0).dot_graph_file).is_null() {
        ...
        let mut symbol_1: *const libc::c_char =
        ...
        while *symbol_1.offset(i as isize) as libc::c_int != '\0' as i32
            && off < 1024 as libc::c_int
        {
            match *symbol_1.offset(i as isize) as libc::c_int {
                9 => {
                    let fresh8 = off;
                    off = off + 1;
                ...
                _ => {
                    let fresh20 = off;
                    off = off + 1;
                    *buf.offset(fresh20 as isize) = *symbol_1.offset(i as isize);
                }
            }
            i += 1;
        }
...
unsafe extern "C" fn ts_parser__recover(
    ...
    *(children_0.contents).offset(fresh24 as isize) = lookahead;
    ...
    let mut has_error: bool = 1 as libc::c_int != 0;
    let mut i_2: libc::c_uint = 0 as libc::c_int as libc::c_uint;
    while i_2 < ts_stack_version_count((*self_0).stack) {
        let mut status: ErrorStatus = ts_parser__version_status(self_0, i_2);
...
unsafe extern "C" fn ts_parser__check_progress(
    mut self_0: *mut TSParser,
    mut lookahead: *mut Subtree,
    mut position: *const uint32_t,
    mut operations: libc::c_uint,
) -> bool {
    (*self_0).operation_count = ((*self_0).operation_count).wrapping_add(operations);
    if (*self_0).operation_count >= OP_COUNT_PER_PARSER_TIMEOUT_CHECK {
        (*self_0).operation_count = 0 as libc::c_int as libc::c_uint;
    }
    if ((*self_0).parse_options.progress_callback).is_some() && !position.is_null() {
        (*self_0).parse_state.current_byte_offset = *position;
        (*self_0).parse_state.has_error = (*self_0).has_error;
    }
    if (*self_0).operation_count == 0 as libc::c_int as libc::c_uint
        && (!((*self_0).cancellation_flag).is_null()
            ...
            || ((*self_0).parse_options.progress_callback).is_some()
                && ((*self_0).parse_options.progress_callback).expect("non-null function pointer")(
                    &mut (*self_0).parse_state,
                ) as libc::c_int
                    != 0)
    {
...
unsafe extern "C" fn ts_parser__balance_subtree(mut self_0: *mut TSParser) -> bool {
    let mut finished_tree: Subtree = (*self_0).finished_tree;
    if !(*self_0).canceled_balancing {
        (*self_0).tree_pool.tree_stack.size = 0 as libc::c_int as uint32_t;
        if ts_subtree_child_count(finished_tree) > 0 as libc::c_int as libc::c_uint
            && (*finished_tree.ptr).ref_count == 1 as libc::c_int as libc::c_uint
        {
            _array__grow(
                &mut (*self_0).tree_pool.tree_stack as *mut MutableSubtreeArray as *mut Array,
                1 as libc::c_int as uint32_t,
                ::core::mem::size_of::<MutableSubtree>() as libc::c_ulong,
            );
            let fresh27 = (*self_0).tree_pool.tree_stack.size;
            (*self_0).tree_pool.tree_stack.size =
                ((*self_0).tree_pool.tree_stack.size).wrapping_add(1);
            *((*self_0).tree_pool.tree_stack.contents).offset(fresh27 as isize) =
                ts_subtree_to_mut_unsafe(finished_tree);
        }
    }
    while (*self_0).tree_pool.tree_stack.size > 0 as libc::c_int as libc::c_uint {
        if !ts_parser__check_progress(
            self_0,
            0 as *mut Subtree,
            0 as *const uint32_t,
        ...
        let mut tree: MutableSubtree = *((*self_0).tree_pool.tree_stack.contents).offset(
            ((*self_0).tree_pool.tree_stack.size).wrapping_sub(1 as libc::c_int as libc::c_uint)
                as isize,
        );
            ...
            if repeat_delta > 0 as libc::c_int as libc::c_long {
                ...
                while i > 0 as libc::c_int as libc::c_uint {
                    ts_subtree_compress(
                        ...
                        i,
                        (*self_0).language,
                        &mut (*self_0).tree_pool.tree_stack,
                    );
                    n = n.wrapping_sub(i);
                    let mut operations: uint8_t =
                        (if i >> 4 as libc::c_int > 0 as libc::c_int as libc::c_uint {
                            i >> 4 as libc::c_int
                        } else {
                            1 as libc::c_int as libc::c_uint
                        }) as uint8_t;
        ...
        (*self_0).tree_pool.tree_stack.size = ((*self_0).tree_pool.tree_stack.size).wrapping_sub(1);
        ...
        while i_0 < (*tree.ptr).child_count {
                ...
                && (*child.ptr).ref_count == 1 as libc::c_int as libc::c_uint
            {
                ...
                let fresh28 = (*self_0).tree_pool.tree_stack.size;
                (*self_0).tree_pool.tree_stack.size =
                    ((*self_0).tree_pool.tree_stack.size).wrapping_add(1);
                *((*self_0).tree_pool.tree_stack.contents).offset(fresh28 as isize) =
...
unsafe extern "C" fn ts_parser_has_outstanding_parse(mut self_0: *mut TSParser) -> bool {
    return (*self_0).canceled_balancing as libc::c_int != 0
        || !((*self_0).external_scanner_payload).is_null()
...
pub unsafe extern "C" fn ts_parser_new() -> *mut TSParser {
    ...
    (*self_0).has_error = 0 as libc::c_int != 0;
    (*self_0).canceled_balancing = 0 as libc::c_int != 0;
    (*self_0).external_scanner_payload = 0 as *mut libc::c_void;
...
pub unsafe extern "C" fn ts_parser_set_language(
    ...
    (*self_0).language = 0 as *const TSLanguage;
    if !language.is_null() {
        if (*language).abi_version > 15 as libc::c_int as libc::c_uint
...
pub unsafe extern "C" fn ts_parser_reset(mut self_0: *mut TSParser) {
    ...
    (*self_0).has_error = 0 as libc::c_int != 0;
    (*self_0).canceled_balancing = 0 as libc::c_int != 0;
    (*self_0).parse_options = {
        let mut init = TSParseOptions {
...
pub unsafe extern "C" fn ts_parser_parse(
    ...
    if ts_parser_has_outstanding_parse(self_0) {
        ...
        if (*self_0).canceled_balancing {
            current_block = 18379371823812163977;
        } else {
        ...
        if (*self_0).has_scanner_error {
            current_block = 10903660011100416690;
        } else {
            if !old_tree.is_null() {
    ...
    match current_block {
        18379371823812163977 => {
            if !((*self_0).finished_tree.ptr).is_null() {
            } else {
                panic!();
            }
            if !ts_parser__balance_subtree(self_0) {
                (*self_0).canceled_balancing = 1 as libc::c_int != 0;
                return 0 as *mut TSTree;
            }
            (*self_0).canceled_balancing = 0 as libc::c_int != 0;
            if ((*self_0).lexer.logger.log).is_some() || !((*self_0).dot_graph_file).is_null() {
...
pub unsafe extern "C" fn ts_parser_parse_with_options(
    ...
    mut old_tree: *const TSTree,
    mut input: TSInput,
    mut parse_options: TSParseOptions,
) -> *mut TSTree {
    (*self_0).parse_options = parse_options;
    (*self_0).parse_state.payload = parse_options.payload;
    let mut result: *mut TSTree = ts_parser_parse(self_0, old_tree, input);
    return result;
}
#[no_mangle]
