...
pub struct TSLanguage {
    ...
    pub max_reserved_word_set_size: uint16_t,
    pub supertype_count: uint32_t,
...
pub unsafe extern "C" fn ts_language_supertypes(
    ...
    if (*self_0).abi_version >= 15 as libc::c_int as libc::c_uint {
        *length = (*self_0).supertype_count;
        return (*self_0).supertype_symbols;
    } else {
        *length = 0 as libc::c_int as uint32_t;
        return 0 as *const TSSymbol;
    };
}
#[no_mangle]
pub unsafe extern "C" fn ts_language_subtypes(
    mut self_0: *const TSLanguage,
    mut supertype: TSSymbol,
    mut length: *mut uint32_t,
) -> *const TSSymbol {
    if (*self_0).abi_version < 15 as libc::c_int as libc::c_uint
        || !(ts_language_symbol_metadata(self_0, supertype)).supertype
    {
        *length = 0 as libc::c_int as uint32_t;
        return 0 as *const TSSymbol;
    }
    let mut slice: TSMapSlice = *((*self_0).supertype_map_slices).offset(supertype as isize);
    *length = slice.length as uint32_t;
    return &*((*self_0).supertype_map_entries).offset(slice.index as isize) as *const TSSymbol;
}
#[no_mangle]
pub unsafe extern "C" fn ts_language_version(mut self_0: *const TSLanguage) -> uint32_t {
    return (*self_0).abi_version;
}
#[no_mangle]
pub unsafe extern "C" fn ts_language_abi_version(mut self_0: *const TSLanguage) -> uint32_t {
    return (*self_0).abi_version;
}
#[no_mangle]
pub unsafe extern "C" fn ts_language_metadata(
    ...
    return if (*self_0).abi_version >= 15 as libc::c_int as libc::c_uint {
        &(*self_0).metadata
    } else {
        0 as *const TSLanguageMetadata
    };
}
#[no_mangle]
pub unsafe extern "C" fn ts_language_name(mut self_0: *const TSLanguage) -> *const libc::c_char {
    return if (*self_0).abi_version >= 15 as libc::c_int as libc::c_uint {
        (*self_0).name
    } else {
...
pub unsafe extern "C" fn ts_language_lex_mode_for_state(
    ...
    if (*self_0).abi_version < 15 as libc::c_int as libc::c_uint {
        let mut mode: TSLexMode = *((*self_0).lex_modes as *const TSLexMode).offset(state as isize);
        return {
            let mut init = TSLexerMode {
                lex_state: mode.lex_state,
                external_lex_state: mode.external_lex_state,
                reserved_word_set_id: 0 as libc::c_int as uint16_t,
            };
            init
        };
    } else {
        return *((*self_0).lex_modes).offset(state as isize);
    };
}
#[no_mangle]
pub unsafe extern "C" fn ts_language_is_reserved_word(
    ...
    let mut lex_mode: TSLexerMode = ts_language_lex_mode_for_state(self_0, state);
    if lex_mode.reserved_word_set_id as libc::c_int > 0 as libc::c_int {
        let mut start: libc::c_uint = (lex_mode.reserved_word_set_id as libc::c_int
            * (*self_0).max_reserved_word_set_size as libc::c_int)
            as libc::c_uint;
        let mut end: libc::c_uint =
            start.wrapping_add((*self_0).max_reserved_word_set_size as libc::c_uint);
        let mut i: libc::c_uint = start;
        while i < end {
            if *((*self_0).reserved_words).offset(i as isize) as libc::c_int
                == symbol as libc::c_int
            {
                return 1 as libc::c_int != 0;
            }
            if *((*self_0).reserved_words).offset(i as isize) as libc::c_int == 0 as libc::c_int {
                break;
            }
            i = i.wrapping_add(1);
        }
...
pub unsafe extern "C" fn ts_lookahead_iterator_new(
    ...
    let mut iterator: *mut LookaheadIterator = crate::core_transpiled::alloc::ts_malloc(
        ::core::mem::size_of::<LookaheadIterator>() as libc::c_ulong,
